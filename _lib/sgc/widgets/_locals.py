...
def update(time):
    ...
    def draw_opengl(widget):
        texture_data = pygame.image.tostring(widget.image, "RGBA")

        w,h = widget.image.get_size()
        tex = glGenTextures(1)
        ...
        glPushMatrix()
        glTranslatef(widget.rect.x, widget.rect.y, 0)
    ...
    if not SCREEN._opengl:
        ...
        for widget in reversed(active_widgets.sprites()):
            draw_opengl(widget)
...
def add_widget(widget, order=None):
    """
