...
class _ScrollHandle(Simple):
    ...
    def _config(self, **kwargs):
        ...
        if "widget" in kwargs:
            self._parent = kwargs["widget"]

    def _draw_base(self):
        img = self._images["image"]
        img.fill(self._parent._settings["col"])
    ...
    def update_pos(self, xy):
        ...
        """
        scroll_bar = getattr(self._parent, "_scroll_%s" % self.xy)
    ...
    def add(self, order=None, fade=True):
        ...
        if self not in special_case:
            special_case.append(self)
        super(_ScrollHandle, self).add(order, fade)

    def remove(self, fade=True):
        try:
            special_case.remove(self)
